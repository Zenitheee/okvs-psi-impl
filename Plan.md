...
- [ ] Keep `volepsi` as a reference and benchmark baseline.
Goal: use the adapted `volepsi` code to validate ideas and compare performance, but keep the final deliverable centered on `volepsi2`.
...
- [ ] Run a structured performance evaluation.
...
- [ ] Build a demo interface in `volepsi2`.
...
- [ ] Document all deviations from the paper.
Goal: clearly state what has been reproduced, what has been simplified, what is still missing, and what temporary changes were made in `volepsi` for testing.

- [ ] Improve engineering quality in `volepsi2`.
...
3. Run benchmarks and compare with `volepsi`.
